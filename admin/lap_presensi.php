...
{
    $postttgl = $_POST['tgl_data'];
    header("location:lap_presensi.php?filter=$postttgl&idrule=$idrule");
}
                            ...
                            <div class="panel panel-default">
								...
								<form action="" method="post" class="form-inline">
								  ...
								  <div class="form-group mx-lg-3 mx-md-2 mx-sm-1 mb-2">
										...
										<?php
										  $jmldatapresensi = $db->cek_get_tgl_presensi($idrule);
										  if($jmldatapresensi!=0){
										  $data_tgl = $db->get_tgl_presensi($idrule);
										  $jmldata = count($data_tgl);
										  foreach($data_tgl as $tglnya){ ?>
											  <option value="<?php echo $tglnya['tgl']; ?>" <?php if($filter==$tglnya['tgl']){ echo "selected";} ?>><?php echo "Pertemuan ke-".$jmldata.'. '.$tglnya['tgl']; ?></option>
										  <?php $jmldata--; } } ?>
										</select>
								  </div>
								  <button name="update" type="submit" class="btn btn-primary btn-flat mb-2">CEK</button>
								</form>
								</div>
							</div>
						<?php } 
                                ...
                                <div class="panel-body">
                                    <div class="table-responsive">
                                        <table class="table table-striped table-bordered table-hover" id="dataTables-example">
                                            ...
                                            <tbody>
												<?php 
													if($cekfilter=="todayok" || $cekfilter=="today"){														
														$hasilcek = "today";
														$mapelnya="";
													}else{
														if($filter!="select"){
													...
													$dataPresensi = $db->tampil_filter_presensi($iduser,$hasilcek,$mapelnya);
